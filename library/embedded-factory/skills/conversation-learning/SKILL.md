...
1. **`.claude/knowledge-base/lessons.jsonl` tail 20** — ostatnie lessons, szczególnie z `category: conversation-learning` lub `triggered_by: conversation-learning-hook`
2. **`.claude/.claude/knowledge-base/candidate-lessons.jsonl`** (jeśli istnieje) — pending HITL candidates, sprawdź `hitl_approved IS NULL` count
...
7. Atomic append candidate JSON do `.claude/.claude/knowledge-base/candidate-lessons.jsonl`
...
**Schema candidate-lessons.jsonl** — własna `.claude/knowledge-base/candidate-lessons-schema.json` (PRE-HITL gate, NIE blokuje lessons.jsonl commit):
...
**HITL gate przez `/review-candidate-lessons`** — operator batch review (default 10/batch), akcje `y/n/e/s/q`. Accept → promotion do lokalnego `.claude/.claude/knowledge-base/lessons.jsonl` (NIE centralna fabryka — promotion to fabryki przez `/promote-lessons` od ).
...
→ candidate appended do .claude/.claude/knowledge-base/candidate-lessons.jsonl
...
```bash
echo '{"ts":"...","actor":"conversation-learning","action":"lesson_logged","artifact":"lessons.jsonl","status":"ok","notes":"pattern: <type>, source: chat"}' >> .claude/knowledge-base/activity-log.jsonl
```
