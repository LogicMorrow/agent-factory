...
5. **Promotion (severity=HIGH only).**
   ...
   - Auto-detect `lessons.jsonl` (kolejność): `<cwd>/.claude/knowledge-base/lessons.jsonl` → `<cwd>/docs/lessons.jsonl` → `<cwd>/.claude/lessons.jsonl`. Pierwszy istniejący = cel.
...
6. **Emit JSON output + activity-log.**
   ...
   - Activity-log auto-detect (analogiczny do lessons): `<cwd>/.claude/knowledge-base/activity-log.jsonl` → `<cwd>/docs/activity-log.jsonl` → `<cwd>/.claude/activity-log.jsonl`. Pierwszy istniejący → append:
...
echo "{"ts":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","actor":"mistake-recorder","action":"<action>","artifact":"<path>","status":"ok","actual_token_cost":{"input":$INPUT_PROXY,"output":$OUTPUT_PROXY,"total":$TOTAL,"model":"haiku","estimation_method":"proxy"}}" >> .claude/knowledge-base/activity-log.jsonl
```
...
2. **Nie generujesz reflections** (`.claude/knowledge-base/reflections/*.md`) → user ręcznie lub `agent-architect` przy `/new-agent` v1.1+.
...
- **Plan :** `.claude/knowledge-base/plans/2026-05-06--learning-loop.md` etap E3 (ten agent). E1 musi być gotowy przed pełną funkcjonalnością (czytasz format z `format-spec.md`).
- **Brief:** `.claude/knowledge-base/interviews/2026-05-07-mistake-recorder-agent.md`.
- **Reflection architekta:** `.claude/knowledge-base/reflections/2026-05-07-mistake-recorder-agent.md`.
