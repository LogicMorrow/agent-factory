...
3. **Post-proposal:** `/version-bumper --proposal=.claude/knowledge-base/improvement-proposals/<file>.md` — wczytujesz konkretną proposal z `/review-lessons` jako prior i obliczasz delta vs aktualny stan agenta.
...
2. Czytaj 3 najnowsze reflections: `Glob: .claude/knowledge-base/reflections/version-bumper*.md` (sort desc, head 3) — skip jeśli brak
3. Czytaj `.claude/knowledge-base/lessons.jsonl` — tail 20 wierszy
...
| `--output-dir` | `.claude/knowledge-base/version-bumper-reports/` | Folder na proposal pliki |
...
2. **Lessons.jsonl filtered:**
   - `Read .claude/knowledge-base/lessons.jsonl` (cały plik, JSONL)
   - Filter:
     - `ts >= since_date`
...
4. **Reflections per agent:**
   - `Glob: .claude/knowledge-base/reflections/<name>*.md` (sort desc, all matches)
   - `Read` każdy plik
...
Output do `.claude/knowledge-base/version-bumper-reports/<YYYY-MM-DD>-<agent>.md`:
...
**Plan:** .claude/knowledge-base/plans/2026-05-13--operationalize-learning-loop.md
```
...
```json
{
  ...
  "proposals_paths": [
    ".claude/knowledge-base/version-bumper-reports/2026-05-XX-cv-builder.md",
    ".claude/knowledge-base/version-bumper-reports/2026-05-XX-offer-analyzer.md",
    ".claude/knowledge-base/version-bumper-reports/2026-05-XX-tech-doc-writer.md"
  ],
...
Activity-log entry:
```bash
echo '{"ts":"<ISO>","actor":"version-bumper","event":"version_proposals_generated","artifact":".claude/knowledge-base/version-bumper-reports/","outcome":"ok","details":{"agents_scanned":33,"proposals_generated":3}}' >> .claude/knowledge-base/activity-log.jsonl
```
...
Zawsze appenduj wpis JSON do `.claude/knowledge-base/activity-log.jsonl` po generacji proposal-i (krok 7).
