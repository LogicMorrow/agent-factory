...
2. Czytaj **wszystkie** poprzednie `.claude/knowledge-base/self-pilot-reports/*.md` (sort desc, head 4) — żeby widzieć trendy week-over-week
3. Czytaj `.claude/knowledge-base/lessons.jsonl` — tail 30 wierszy (więcej niż standard 20 bo self-pilot dotyczy całej fabryki)
...
| `--output` | `.claude/knowledge-base/self-pilot-reports/<date>.md` | Plik report |
...
previous_reports = glob('.claude/knowledge-base/self-pilot-reports/*.md')
...
for agent in this_week_pilot:
    ...
    fixtures_dir = f".claude/knowledge-base/fixtures/{agent}/"
...
`Write .claude/knowledge-base/self-pilot-reports/<YYYY-MM-DD>-weekly.md`:
...
Output dir: `.claude/knowledge-base/self-pilot-reports/version-proposals/<date>/`
...
```json
{
  ...
  "dod_score": "12/14",
  "report_path": ".claude/knowledge-base/self-pilot-reports/2026-05-20-weekly.md",
...
Activity-log:
```bash
echo '{"ts":"<ISO>","actor":"self-pilot","action":"self_pilot_run","artifact":".claude/knowledge-base/self-pilot-reports/<date>-weekly.md","status":"ok","notes":"weekly: 2 proposals, 1 pilot, DoD 12/14"}' >> .claude/knowledge-base/activity-log.jsonl
```
